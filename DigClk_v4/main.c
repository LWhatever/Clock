...
#include "msp430f6638.h"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             #include <msp430f6638.h>
#include "oled.h"
#include "bmp.h"
/*
 * main.c
...
int month_mode = 1, set_mode = 0, set_alarm = 0, display = 1, LH = 1;//月份天数判断，时间，闹钟设置
...
void flash()
{
	switch(set_mode)
	{
		case 0:break;
		case 1:                                              //年份的千百位闪烁
			OLED_ShowChar(0+24, 0, ' ');
			OLED_ShowChar(8+24, 0, ' ');__delay_cycles(1000000);
			OLED_ShowNum(8+24, 0, (year-(year/1000)*1000)/100, 1, 16);
			OLED_ShowNum(0+24, 0, year/1000, 1, 16);break;
		case 2:                                              //年份的十个位闪烁
			OLED_ShowChar(16+24, 0, ' ');
			OLED_ShowChar(24+24, 0, ' ');__delay_cycles(1000000);
			OLED_ShowNum(24+24, 0, year%10, 1, 16);
			OLED_ShowNum(16+24, 0, (year-(year/100)*100)/10, 1, 16);
			break;
		case 3: 											//月闪烁
...
void set_time()
{
	switch(set_mode)
	{
		...
		case 1:
			if(LH==1)
				year = year - (year/100)*100 + KeyVal*1000;
			else
				year = year + KeyVal*100;
			break;
		case 2:
			if(LH==1)
				year = year - (year%100) + KeyVal*10;
			else
				year = year + KeyVal;
			break;
		case 3:
			if(LH==1)
			{
				month = month - (month/10)*10 + KeyVal*10;
				LH = (month>12)?-1:1;
			}
			else
				month = month - month%10 + KeyVal;
			break;
		case 4:
			if(LH==1)
			{
				day = day - (day/10)*10 + KeyVal*10;
				LH = (day>31)?-1:1;
			}
			else
				day = day - day%10 + KeyVal;
			break;
		case 5:
			if(LH==1)
			{
				hour = hour - (hour/10)*10 + KeyVal*10;
				LH = (hour>23)?-1:1;
			}
			else
				hour = hour - hour%10 + KeyVal;
			break;
		case 6:
			if(LH==1)
			{
				minute = minute - (minute/10)*10 + KeyVal*10;
				LH = (minute>59)?-1:1;
			}
			else
				minute = minute - minute%10 + KeyVal;
			break;
		default:break;
	}
...
void Set_Alarm()
{
	switch(set_alarm)
	{
		...
		case 1:
			if(LH==1)
				alarm[0] = alarm[0] - (alarm[0]/10)*10 + KeyVal*10;
			else
				alarm[0] = alarm[0] - alarm[0]%10 + KeyVal;
			break;
		case 2:
			if(LH==1)
				alarm[1] = alarm[1] - (alarm[1]/10)*10 + KeyVal*10;
			else
				alarm[1] = alarm[1] - alarm[1]%10 + KeyVal;
			break;
		case 3:
			if(LH==1)
				alarm[3] = alarm[3] - (alarm[3]/10)*10 + KeyVal*10;
			else
				alarm[3] = alarm[3] - alarm[3]%10 + KeyVal;
			break;
		case 4:
			if(LH==1)
				alarm[4] = alarm[4] - (alarm[4]/10)*10 + KeyVal*10;
			else
				alarm[4] = alarm[4] - alarm[4]%10 + KeyVal;
			break;
		case 5:
			if(LH==1)
				alarm[6] = alarm[6] - (alarm[6]/10)*10 + KeyVal*10;
			else
				alarm[6] = alarm[6] - alarm[6]%10 + KeyVal;
			break;
		case 6:
			if(LH==1)
				alarm[7] = alarm[7] - (alarm[7]/10)*10 + KeyVal*10;
			else
				alarm[7] = alarm[7] - alarm[7]%10 + KeyVal;
			break;
		default:break;
	}
	Adjust_clock_al();
...
void ADC_Init()
{
	...
	ADC12MCTL0 |= ADC12INCH_10; //选择通道15，连接拨码电位器
	ADC12CTL0 |= ADC12ENC;
}
...
__interrupt void TIMER1_A1_ISR(void)
{
	...
	switch(KeyVal)
	{
		...
		case 14:								//E 停止闹钟
			...
			break;
		case 15:								//F 开关闹钟
			alarm[2] = (set_alarm==1|set_alarm==2)?-alarm[2]:alarm[2];
